...
export async function extractSchemasWithPuppeteer(url) {
  ...
  try {
    ...
    if (isVercel || !puppeteer || !puppeteer.default?.executablePath) {
      // Serverless/runtime-compatible: @sparticuz/chromium (Vercel or environments without bundled Chromium)
      ...
      // Local development: Prefer Puppeteer's bundled Chromium when available (works on macOS dev)
      console.log(`   → Using Puppeteer bundled Chromium for local development`);
