...
import { expandKeyword, detectAudienceType } from './keyword-expander.js';
...
async function analyzeEntry(entry) {
  ...
  try {
    ...
    // Step 2: Expand keyword into prompts (using location and audience context from entities)
    ...
    const audienceContext = detectAudienceType(entities.entities || {});
    if (audienceContext.signals.length > 0) {
      console.log(`   ✓ Detected audience: ${audienceContext.audienceType} (${audienceContext.signals.join(', ')})`);
    }

    let keywordExpansion;
    try {
      keywordExpansion = await expandKeyword(entry.keyword, 5, location, audienceContext);
    } catch (error) {
