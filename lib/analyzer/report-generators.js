...
export function generateReportCard(analysis) {
  ...
  const brand = entities.pageData?.organizationName || extractBrandName(url);
...
export function exportReportCardMarkdown(reportCard, outputDir) {
  const filename = `report-card-${sanitizeFilename(reportCard.brand)}.md`;
  ...
  const locationDisplay = `**Location:** ${locationText}\n`;

  const markdown = `# AI Search Readiness Report Card

**Brand:** ${reportCard.brand}
...
- **Cited:** ${checks.googleAIOverview.cited ? '✅ Yes' : '❌ No'}${checks.googleAIOverview.position ? ` (Position #${checks.googleAIOverview.position})` : ''}${checks.googleAIOverview.matchType ? ` — ${checks.googleAIOverview.matchType === 'exact' ? 'Exact' : checks.googleAIOverview.matchType === 'partial' ? 'Partial (same site)' : 'No match'}` : ''}
...
${checks.googleAIOverview.citations.slice(0, 5).map((c, idx) => {
  const isObj = typeof c === 'object' && c !== null;
  const url = isObj ? c.url : c;
  const match = isObj ? c.match : undefined;
  const color = match === 'exact' ? '#d1fae5' : match === 'partial' ? '#fef9c3' : 'transparent';
  const label = match === 'exact' ? 'Exact' : match === 'partial' ? 'Partial' : 'Other';
  return `${idx + 1}. <span style="background-color: ${color}; padding:2px 4px; border-radius:3px; word-break: break-all; overflow-wrap: anywhere;">${url}${match ? ` — ${label}` : ''}</span>`;
...
- **Cited:** ${checks.perplexity.cited ? '✅ Yes' : '❌ No'}${checks.perplexity.position ? ` (Position #${checks.perplexity.position})` : ''}${checks.perplexity.matchType ? ` — ${checks.perplexity.matchType === 'exact' ? 'Exact' : checks.perplexity.matchType === 'partial' ? 'Partial (same site)' : 'No match'}` : ''}
...
${checks.perplexity.citations.slice(0, 5).map((c, idx) => {
  const isObj = typeof c === 'object' && c !== null;
  const url = isObj ? c.url : c;
  const match = isObj ? c.match : undefined;
  const color = match === 'exact' ? '#d1fae5' : match === 'partial' ? '#fef9c3' : 'transparent';
...
- **Cited:** ${checks.chatgpt.cited ? '✅ Yes' : '❌ No'}${checks.chatgpt.position ? ` (Position #${checks.chatgpt.position})` : ''}${checks.chatgpt.matchType ? ` — ${checks.chatgpt.matchType === 'exact' ? 'Exact' : checks.chatgpt.matchType === 'partial' ? 'Partial (same site)' : 'No match'}` : ''}
...
${checks.chatgpt.citations.slice(0, 5).map((c, idx) => {
  const isObj = typeof c === 'object' && c !== null;
  const url = isObj ? c.url : c;
  const match = isObj ? c.match : undefined;
  const color = match === 'exact' ? '#d1fae5' : match === 'partial' ? '#fef9c3' : 'transparent';
...
function generateRecommendations(analysis) {
  ...
  if (entities.entities?.semanticScore?.entityDensity < 60) {
    recommendations.push({
      ...
      steps: [
        'Clearly mention product/service names, credentials, and certifications',
        'Add organization name and location throughout content',
        'Include team member names and credentials',
        'Reference specific features, offerings, or details',
      ],
    });
  }
  ...
  if (entities.entities?.semanticScore?.eeatScore < 60) {
    recommendations.push({
      ...
      steps: [
        ...
        'Display certification/accreditation badges prominently',
        'Include outcome statistics and testimonials',
...
/**
 * Generate cost breakdown section for report
 */
function generateCostSection() {
  const summary = costTracker.getSummary();
  const { totalCost, breakdown } = summary;

  let section = `**Estimated Total Cost:** $${totalCost.toFixed(4)}\n\n`;
  section += `This analysis used the following API services:\n\n`;

  if (breakdown.anthropic.requests > 0) {
    section += `- **Anthropic Claude** (${breakdown.anthropic.requests} requests)\n`;
    section += `  - Input tokens: ${breakdown.anthropic.inputTokens.toLocaleString()}\n`;
    section += `  - Output tokens: ${breakdown.anthropic.outputTokens.toLocaleString()}\n`;
    section += `  - Cost: $${breakdown.anthropic.cost.toFixed(4)}\n\n`;
  }

  if (breakdown.perplexity.requests > 0) {
    section += `- **Perplexity AI** (${breakdown.perplexity.requests} requests)\n`;
    section += `  - Cost: $${breakdown.perplexity.cost.toFixed(4)}\n\n`;
  }

  if (breakdown.openai.requests > 0) {
    section += `- **OpenAI GPT-4** (${breakdown.openai.requests} requests)\n`;
    section += `  - Input tokens: ${breakdown.openai.inputTokens.toLocaleString()}\n`;
    section += `  - Output tokens: ${breakdown.openai.outputTokens.toLocaleString()}\n`;
    section += `  - Cost: $${breakdown.openai.cost.toFixed(4)}\n\n`;
  }

  if (breakdown.serpapi.requests > 0) {
    section += `- **SerpAPI** (${breakdown.serpapi.requests} searches)\n`;
    section += `  - Cost: $${breakdown.serpapi.cost.toFixed(4)}\n\n`;
  }

  if (breakdown.dataforseo.requests > 0) {
    section += `- **DataForSEO** (${breakdown.dataforseo.requests} requests)\n`;
    section += `  - Cost: $${breakdown.dataforseo.cost.toFixed(4)}\n\n`;
  }

  section += `*Note: Prices are estimates based on January 2025 pricing and may vary.*`;
