...
export async function expandKeyword(keyword, count = 5, location = null) {
  ...
  if (shouldAddLocation) {
    locationContext = `\nInstitution Location: ${location.city ? location.city + ', ' : ''}${location.state || ''}${location.region ? ' (' + location.region + ')' : ''}`;
  }

  const prompt = `You are an expert in search intent and higher education marketing.

Given this keyword: "${keyword}"${locationContext}

Generate ${count} natural question variations that prospective students would ask AI search engines (ChatGPT, Perplexity, Google AI Overviews, etc.).

IMPORTANT: It is currently late 2025. If you include a year in questions, use 2025 or 2026, NOT 2024.

${shouldAddLocation ? `LOCATION CONTEXT: Since the keyword doesn't include location but the institution is located in ${location.city || location.state}, generate a mix of prompts:
...
- navigational: looking for a specific institution
- comparison: comparing options
- transactional: ready to apply/enroll
